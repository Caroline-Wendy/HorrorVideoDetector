...
void test (void)
{
	...
	unsigned int num = 
		HorrorVideoSceneNum(fileName.c_str(), shotInterval, sceneShotNum);

	double* resultSet = new double[num];

	if (HorrorVideoDetector(resultSet, fileName.c_str(), modelPath.c_str(), 
...
void test_image (void) 
{
	...
	unsigned int num = HorrorVideoSceneNum(fileName.c_str(), 
		shotInterval, sceneShotNum);
		...
		modelPath.c_str(), shotInterval, sceneShotNum)) 
	{
		for (unsigned int i=0; i<num; ++i) {
			...
			std::cout << " Scene[" << i << "] Begin Time : " 
				<< resultSet[i].btime << std::endl;
